...
def read_py(A,V,Y):
    if str(A) == 'veg_options':
        ans = raw_input("Are you looking for vegetarian food options? [yes/no] ")
        Y.unify(ans)
    return True

def read_py_menu8(A,V,Y):
    ans = raw_input("Choose a " + str(A) + ". Select 1 for " + str(V[0]) + ', 2 for ' + str(V[1]) + ', 3 for ' + str(V[2]) + ', 4 for ' + str(V[3]) + ', 5 for ' + str(V[4]) + ', 6 for ' + str(V[5]) + ', 7 for ' + str(V[6]) + ', or 8 for ' + str(V[7]) + ". ")
    if ans == '1':
        Y.unify(str(V[0]))
    elif ans == '2':
        Y.unify(str(V[1]))
...
def read_py_menu3(A,V,Y):
    ans = raw_input("Choose a " + str(A) + ". Select 1 for " + str(V[0]) + ', 2 for ' + str(V[1]) + ', 3 for ' + str(V[2]) + ". ")
    if ans == '1':
        Y.unify(str(V[0]))
    elif ans == '2':
        Y.unify(str(V[1]))
...
def read_py_menu2(A,V,Y):
    Y.unify(raw_input("Choose a " + str(A) + " from [" + str(V[0]) + ', ' + str(V[1]) + "]? "))
    return True
